...
class GANModelDesc(ModelDescBase):
    def collect_variables(self, g_scope='gen', d_scope='discrim'):
        ...
        for y in self.g_vars:
            if not 'senc/senc' in y.name:
                self.g_vars_mines_e.append(y)
        assert self.g_vars
        assert self.g_vars_mines_e
...
class GANTrainer(TowerTrainer):
    ...
    def _build_multigpu_gan_trainer(self, input, model, num_gpu):
        ...
        def get_cost(*inputs):
            ...
            return [model.d_loss, model.g_loss, model.z_loss]
        ...
        with tf.name_scope('optimize'):
            ...
            z_loss = tf.add_n([x[2] for x in cost_list]) * (1.0 / num_gpu)
            ...
            with tf.control_dependencies([g_min]):
                g_min_e = opt.minimize(z_loss, var_list=model.g_vars_mines_e,
